...
| Secret | Origin | Why it is not managed here |
|--------|--------|----------------------------|
| `cattle-system/bootstrap-secret` | generated by the Rancher chart on first install | Rancher regenerates and consumes it during initial admin setup; committing it would fight the controller and serve no purpose once the admin password is set |
