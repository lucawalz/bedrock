...
Velero's scope shrinks to the data that genuinely needs off-cluster capture. It no longer snapshots the Postgres volumes, the monitoring telemetry, or the model weights, which reduces backup size and window.

**Correction, 2026-07-25.** The de-duplication described here was never achieved. The Velero half worked: `inheritedMetadata` stamps `velero.io/exclude-from-backup` on the Postgres PVCs and Velero honours it. The Longhorn half was overlooked entirely. Longhorn assigns every volume carrying no recurring-job label to the `default` group, and the `backup` RecurringJob targets that group, so all three Postgres data volumes continued to be captured crash-consistent every night at 03:30, which is precisely what this record claimed to have stopped. The exclusion label means nothing to Longhorn.

A second duplication was introduced rather than removed. The `longhorn-snapshot-vsc` VolumeSnapshotClass carries no `parameters`, so Longhorn's CSI driver defaults to producing a backup in the object store rather than a local snapshot. Every volume Velero does snapshot is therefore written to `basalt-backups` twice a night, once by the RecurringJob at 03:30 and once through Velero's CSI path at 02:00.

Both are corrected by labelling the Postgres PVCs out of the `default` group through `inheritedMetadata`, which requires `recurring-job.longhorn.io/source: enabled` before Longhorn reads PVC labels at all. Barman is now the sole mechanism for Postgres, as this record originally intended. The 21 Longhorn backups taken under the old behaviour were deleted on 2026-07-25, because a volume that leaves the group is never selected by the recurring job again and its backups would otherwise never be pruned.

**Correction, 2026-07-29.** The sentence above claiming both duplications are corrected holds for the first only, because labelling the Postgres volumes out of the `default` group does nothing about the VolumeSnapshotClass.

The Postgres half is confirmed fixed. The three instance volumes carry only `recurring-job.longhorn.io/filesystem-trim` and `recurring-job.longhorn.io/snapshot-prune`, sit outside the `default` group, and Barman is the sole mechanism for Postgres as this record originally intended.

The second duplication is still live. `longhorn-snapshot-vsc` reports empty `parameters`, so every non-Postgres volume Velero snapshots is written to `basalt-backups` twice a night, once by the RecurringJob at 03:30 and once through the CSI path at 02:00. Closing it means setting the `type` parameter in `kubernetes/infrastructure/controllers/storage/velero/volumesnapshotclass.yaml`, and the change should be made deliberately rather than casually, because moving that path from a backup to a local snapshot removes an off-cluster copy that currently exists.

One side effect of the labelling is worth recording: `kiwix-library` also carries only the trim and prune labels, so it sits outside the backup group as well. That matches the accepted restore-by-redownload trade for that volume rather than being an oversight, but it means `LonghornVolumeNeverBackedUp` fires permanently for it and for the Postgres volumes, all of which are excluded by design.

The separate credential described above is also not what this record claims. The `cnpg-backup-s3` key is a different key pair from the one Velero, Longhorn and k3s share, but Hetzner key pairs are project-wide by default and no bucket policy exists on either bucket. Both keys carry `FULL_CONTROL` over both buckets, which was confirmed by using the CNPG key to list `basalt-backups` and read its ACL. The separation is therefore organisational, not enforced, and the claimed blast-radius reduction does not hold. Scoping it needs a bucket policy naming the access key, which is a console action recorded as an open gap in the recovery runbook. Because Velero runs with the node agent disabled, its volume durability for what remains rides Longhorn's BackupTarget writing to the shared bucket, so the CSI snapshot path and the Longhorn backup are the real durability mechanism for non-Postgres volumes; the Velero exclusions change what is offered to that path, not a separate copy. The Ollama vision volume move is a one-time model re-pull on the disposable class, accepted because the weights are re-derivable by design.
