...
Velero's scope shrinks to the data that needs off-cluster capture. It no longer snapshots the Postgres volumes, the monitoring telemetry, or the model weights, which reduces backup size and window.

**Correction, 2026-07-25.** The de-duplication described here was never achieved. The Velero half worked: `inheritedMetadata` stamps `velero.io/exclude-from-backup` on the Postgres PVCs and Velero honours it. The Longhorn half was overlooked entirely. Longhorn assigns every volume carrying no recurring-job label to the `default` group, and the `backup` RecurringJob targets that group. All three Postgres data volumes therefore continued to be captured crash-consistent every night at 03:30, which is precisely what this record claimed to have stopped. The exclusion label means nothing to Longhorn.
