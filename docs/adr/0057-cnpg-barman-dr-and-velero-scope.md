...
Both are corrected by labelling the Postgres PVCs out of the `default` group through `inheritedMetadata`, which requires `recurring-job.longhorn.io/source: enabled` before Longhorn reads PVC labels at all. Barman is now the sole mechanism for Postgres, as this record originally intended. The Longhorn backups taken under the old behaviour are not pruned when a volume leaves the group, so they persist in the backup target until deleted deliberately. Because Velero runs with the node agent disabled, its volume durability for what remains rides Longhorn's BackupTarget writing to the shared bucket, so the CSI snapshot path and the Longhorn backup are the real durability mechanism for non-Postgres volumes; the Velero exclusions change what is offered to that path, not a separate copy. The Ollama vision volume move is a one-time model re-pull on the disposable class, accepted because the weights are re-derivable by design.
