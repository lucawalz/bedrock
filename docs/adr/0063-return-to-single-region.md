...
Nothing is preserved outside the working tree. The `multi-region-fleet` tag that once held the spoke tunnel secrets on the secrets repository was removed, because a fleet the estate no longer runs is a remnant rather than an asset. The demonstrated architecture stays recoverable from git history at `2ee65d8e`, the last commit before the reversion, and its reasoning stays in the record as [0061](0061-multi-region-fleet-layout.md).

**Update (2026-09-06):** two things this record kept are gone, and neither loss touches its decision. The backups and disaster recovery it preserved were removed with the Hetzner account by [0081](0081-retire-the-hetzner-account.md), so "the backups, disaster recovery, and hardening persist unchanged" now holds for the hardening alone. On-demand capacity through horizon went with the same account, so the Implementation's "on-demand capacity beyond the three bare-metal nodes remains provided by horizon through the hcloud API" no longer holds either. The return to a single region, the collapse of the profile split and the removal of the Cluster API substrate all stand as written.
