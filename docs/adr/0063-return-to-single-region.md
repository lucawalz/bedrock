...
The spoke tunnel secrets are preserved outside the working tree as the `multi-region-fleet` git tag on the secrets repository. The public repository carries no such tag, and the demonstrated architecture stays recoverable from git history alone, at `2ee65d8e`, the last commit before the reversion.
