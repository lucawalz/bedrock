---
status: accepted, addressing superseded by 0016
date: 2026-06-12
---
...
The cluster was originally numbered `192.168.20.0/24`. The concrete zoned addressing that replaced it is recorded in [0016](0016-concrete-zoned-ip-scheme.md), together with the later split of this VLAN into a trusted servers zone and a separate DMZ for any public-facing host. That record supersedes the addressing here.
...
The cluster is contained: a breach in a public service cannot pivot into personal devices without crossing a rule that denies it. The cost is that the cluster zone depends on the layer-3 router, and reaching it from the home LAN needs a static route to the cluster subnet via the Pi. That extra hop is the price of the boundary. The original decision to isolate the cluster on its own VLAN stands; only the addressing and the introduction of a second DMZ zone have moved on, as captured in [0016](0016-concrete-zoned-ip-scheme.md).
