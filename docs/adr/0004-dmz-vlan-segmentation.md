...
The cluster is contained: a breach in a public service cannot pivot into personal devices without crossing a rule that denies it. The cost is that the cluster zone depends on the layer-3 router, and reaching it from the home LAN needs a static route to the cluster subnet via the Pi. That extra hop is the price of the boundary.
