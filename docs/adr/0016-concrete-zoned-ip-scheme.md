...
The scheme also had to avoid collisions. The `10.0.0.0/24` default is the most common range in consumer gear and cloud defaults. The burst nodes ride a WireGuard tunnel from Hetzner, so an overlap between the home addressing and a cloud network or another VPN would silently break routing across that tunnel.
