...
The crash-looping autoscaler, the Rancher double-import, and the dependency on an unreleased CAPH all disappear, replaced by one autoscaler Deployment and one SOPS secret. That secret carries the per-pool config (labels, taints, cloud-init) injected as environment variables. The helm-controller does not restart the Deployment when the secret's content changes, so a config edit would otherwise sit reconciled but inert until a manual restart. The Deployment therefore carries the Reloader auto-annotation from [0020](0020-reloader-config-driven-rollouts.md) so it rolls itself when the config secret changes. The chart exposes no Deployment-level annotation field, so the annotation is applied through a HelmRelease post-renderer patch.

The orphan-node reaper from [0026](0026-orphan-node-reaper.md) is kept rather than removed, but narrowed to its Longhorn-finalize pass. The provider's clean scale-down deletes the server and the Node object together, so the reaper no longer needs to chase orphan Node records. An uncleanly terminated burst node still strands a `nodes.longhorn.io` record that the native provider does not clean up, which is the case [0040](0040-reap-orphaned-longhorn-nodes.md) added the finalize pass for, so that pass stays. The node-pool shape moves from MachineDeployment manifests to autoscaler flags in the HelmRelease, which is simpler. The home cluster autoscales itself with no operator action, and on-demand bursts go through horizon talking to hcloud directly. Removing the old stack has its own ordering constraint: the ExternalControlPlane-backed `burst` Cluster must be deleted before its control-plane CRD is removed. Deleting the CRD first leaves the Cluster's finalizer unable to resolve and wedges the deletion. The change must be verified with a scale-from-zero test: a pod that only an elastic burst node can host triggers a server create, and the node joins and the pod schedules. Removing the pod returns the group to zero and deletes the server. The cost is a refactor of the node image join path and the loss of the single-API-for-every-cloud framing, which AWS retains on its own merits under [0042](0042-ephemeral-eks-autoscaling-and-s3-foothold.md).
