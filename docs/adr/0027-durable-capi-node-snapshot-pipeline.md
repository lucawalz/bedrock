...
CAPH provisions every burst node from a single Hetzner snapshot named `bedrock-capi-node`, built by the `build-capi-node-snapshot` workflow with Packer and nixos-anywhere. The workflow rebuilt only when a node-image input changed (`flake.nix`, `flake.lock`, `modules/k3s/capi-node.nix`, `modules/k3s/hetzner-scaffolding.nix`, `infra/packer/**`), keyed on a tree hash carried as the `bedrock-nixos-hash` label, and a cleanup job deleted every managed snapshot whose hash did not match the current one.
