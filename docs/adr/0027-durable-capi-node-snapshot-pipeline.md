...
CAPH provisions every burst node from a single Hetzner snapshot named `bedrock-pool-node`, built by the `build-pool-node-snapshot` workflow with Packer and nixos-anywhere. The workflow rebuilt only when a node-image input changed (`flake.nix`, `flake.lock`, `modules/k3s/pool-node.nix`, `modules/k3s/hetzner-scaffolding.nix`, `infra/packer/**`), keyed on a tree hash carried as the `bedrock-nixos-hash` label. A cleanup job deleted every managed snapshot whose hash did not match the current one.

Two properties of that design could leave zero usable images, and with no image CAPH cannot provision any node. The pipeline only ran on input changes, so a static config produced no fresh build for arbitrarily long while the lone snapshot aged, and any out-of-band loss had nothing to recreate it. The prune kept only the current-hash snapshot with no count floor, so a single mistaken evaluation could remove the last image. It also ran on `needs.build.result == 'success'` even when the build job succeeded by skipping (snapshot already present) without confirming the named snapshot actually existed in Hetzner. The 2026-06-15 provisioning outage traced to this class of failure.
...
The forced miss is removed. Packer derives `snapshot_name` from the same tree hash that the existence check queries. Forcing the check to report a miss on a scheduled run made Packer attempt a snapshot whose name already existed, and it exited non-zero before creating a server. Every scheduled run from 22 June 2026 onward failed this way in under a minute. Because the build job failed, the verification gate in the cleanup job was skipped, so promotion and pruning never ran either. The freshness guarantee therefore produced nothing, and it disabled the safeguards it was meant to protect.

Freshness now comes from input churn rather than from a forced rebuild. Renovate bumps `flake.lock` on its normal cadence, which changes the tree hash and triggers a build through the existing push trigger, so the image tracks nixpkgs without a scheduled rebuild. The weekly schedule is kept and is a no-op for the build when inputs are unchanged. That is what lets it still exercise verification, promotion, and pruning, and lets it rebuild for real when an out-of-band loss leaves no snapshot for the current hash. That covers both failure paths the original decision was written against, without the collision.

Two smaller corrections come with it. The cron is set to `0 4 * * 1`, matching the Monday 04:00 UTC this record already specified; it had drifted to 05:00. A `pull_request` trigger scoped to the Packer templates and the workflow runs `packer init`, `packer fmt -check`, and `packer validate`, so a template change is checked before it reaches `main`. The tree-hash computation moved into `scripts/nixos-image-hash.sh` and is used by both jobs, since it was duplicated between them and a divergence would silently build one image and verify another.
