...
Storage is Longhorn, installed through Flux from a Helm chart with a default replica count of three, so every volume has a copy on each node and the loss of one node does not lose data. Longhorn is the default StorageClass and manages its disks on each node.

## Options considered

- Longhorn, chosen. Cluster-native replicated block storage with snapshots and a default replica count of three, run as a Helm release and operated through its own UI.
...
Volumes survive a node failure and can be snapshotted and restored in place. The cost is replication overhead: three copies of every volume use disk and network, and Longhorn needs some tuning to behave well on small hardware. Nothing carries a Longhorn snapshot off-cluster any more, because [0081](0081-retire-the-hetzner-account.md) removed Velero and the Longhorn BackupTarget along with the Hetzner account. A nightly `snapshot` recurring job still runs, but it rides the same volume and the same three replicas as the data it protects, which is not a backup.
