...
Two costs follow. The membership lists above drift as concerns are added or retired, so this record needs amending when they do; the rule in the Decision does not drift and is the part worth keeping accurate. And the foundation is a single ordering bottleneck: `cluster-sources` not being ready holds almost every other Kustomization behind it, which shows up as a wall of dependency-not-ready messages until cluster-sources applies.

Because of that bottleneck, `cluster-sources` reconciles with `wait: false` and reports ready once the source objects are applied rather than once every repository has fetched its index. Health-gating the estate on more than twenty upstream chart servers made any one slow index enough to stall everything new, which a repository served at a few kilobytes per second duly did. A HelmRelease whose repository is unready fails on its own, names the repository in its message, and retries, so the failure stays with the app that wants the chart. [0033](0033-adopt-external-controlplane-object.md) records the same lesson one layer down, where a `wait: true` Kustomization health-gated on the very object it was meant to create and deadlocked on the first apply.
