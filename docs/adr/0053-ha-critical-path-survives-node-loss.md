...
**Correction, 2026-07-29.** The CoreDNS paragraph in the Decision, and the matching entry under Options considered, are superseded by [0065](0065-coredns-flux-high-availability.md). CoreDNS runs two replicas with a required anti-affinity on `kubernetes.io/hostname` and a PodDisruptionBudget, so it no longer depends on a single addon pod rescheduling from master.

A second round of failure injection on 2026-07-29, during the rollout of a new nixpkgs lock, refined the MetalLB finding above. Rebooting master rather than powering it off showed the load balancer address kept serving for roughly the first fifty seconds and then failed for about seventy-five, so the speaker carries on announcing for about a minute after the API server goes before it goes blind. A short reboot therefore spends most of its outage in that dead window, and the earlier description of the address being unreachable for the whole outage holds for a power cycle rather than a reboot.
