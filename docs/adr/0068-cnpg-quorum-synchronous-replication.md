...
**Update (2026-09-06):** the Context's "Barman archives WAL continuously to object storage and gives point-in-time recovery with a recovery point measured in minutes" no longer describes the estate. [0081](0081-retire-the-hetzner-account.md) removes the archiver with the Hetzner account, so there is no recovery mechanism sitting behind the commit guarantee any more. The separation this record draws between durability and recovery still holds and now matters more, not less: the quorum commit is the only thing standing between a primary loss and lost transactions, and it protects against nothing else, because a bad write reaches every replica.
