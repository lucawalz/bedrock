...
kube-router, already the cluster's CNI, enforces the NetworkPolicies, so no extra policy engine is introduced. Runtime threat detection was part of the initial baseline through Falco, deployed on all nodes with the eBPF driver, and has since been removed, so the current baseline rests on the other layers. Firmware patching stays a manual task.
...
A breach is contained on several fronts at once: lateral movement is limited by namespace policies, and a compromised container has fewer capabilities and, for cloudflared, no writable root. Secrets are unreadable from an etcd snapshot alone. The host baseline narrows the node surface too: the cluster is key-only over SSH and its kubeconfig is no longer world-readable. The baseline is injected by the flake builder rather than imported per host, so a newly added node inherits it by construction instead of by remembering to wire it in. The router stays the exception, cherry-picking the baseline modules it needs around its own boot and network stack. The deferrals are recorded honestly and stay open work: NetworkPolicies for `traefik` and `postgres`, full non-root for the `llm` workloads, firmware patching for `rpi-eeprom` and the m920q BIOS, and the permissions on the server PKI under `/var/lib/rancher/k3s/server/tls`. Firmware patching stays a manual chore with no automation behind it. K3s regenerates those certificates and resets their modes on its own schedule, so any chmod or tmpfiles override is undone. Tightening them needs a K3s-native mechanism and is left as a separate decision. The baseline relies on kube-router continuing to enforce policy.
