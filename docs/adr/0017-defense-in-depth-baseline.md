...
The standing risk is that the whole policy layer rests on kube-router continuing to enforce it, and nothing else in the cluster would notice if it stopped. The router pays the one structural cost, because it cannot take the host baseline wholesale and instead cherry-picks the modules it needs around its own boot and network stack. Three deferrals stay open: full non-root for the `llm` workloads, firmware patching for `rpi-eeprom` and the m920q BIOS, which has no automation behind it, and the permissions on the server PKI under `/var/lib/rancher/k3s/server/tls`. K3s regenerates those certificates and resets their modes on its own schedule, so any chmod or tmpfiles override is undone; tightening them needs a K3s-native mechanism and is left as a separate decision.
