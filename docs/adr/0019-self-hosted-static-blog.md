...
Amendment: the deferred Flux Image Update Automation has since been adopted; see ADR 0022. The Deployment had been referencing the floating `latest` tag rather than a pinned one, so the manual tag-bump model was never actually in effect.
