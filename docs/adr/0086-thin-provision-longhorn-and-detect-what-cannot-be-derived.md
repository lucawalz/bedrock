...
over two days, while the disks were roughly a third full. Longhorn schedules against a volume's
**declared** size: a replica's full declaration is charged to a disk's budget the moment it is
placed, whatever the volume contains, and that budget is
...
empty. The reported symptom was not that ceiling: zot was attached and healthy, and
`longhorn-disposable` sets `dataLocality: best-effort`, so Longhorn had created a second replica
record for a copy local to the pod on worker-1, where the 85.9 GB it declared did not fit. The volume
was never at risk, and that unsatisfiable preference was the only thing in the estate saying anything.
...
disk genuinely fills whatever has been promised to volumes that will never use it. The table above
shows every node well inside that floor.
...
headroom just created. Zot is a pull-through cache holding 2.5 GB under a retention policy that has
bounded it correctly since [0067](0067-pull-through-registry-cache.md)'s second update, so 20Gi is
eight times its settled working set, reached by the recreate that record calls the right remedy for a
...
`longhorn:node_replica_scheduling:budget_used_percent`, holds the whole basis so it appears once.

**Detect the conditions no expression can be derived for.** Longhorn exports no metric for a volume's
`Scheduled` condition, and its robustness and per-disk schedulability both read healthy throughout.
...
`LonghornVolumeOverDeclared` measures the declaration-to-contents gap nothing measured before, and
takes two guards because either alone is wrong: a declaration more than ten times the contents **and**
a gap costing over 20 GB of scheduling budget once multiplied by the replica count, since a ratio
fires on every freshly created volume where nothing is wasted and an absolute gap fires on a large
volume in genuine use. It demands no action, because every correction is a claim recreated with its
data migrated.

## Options considered

- Thin provision, and pair the derived alerts with ones reading Longhorn's own verdict, chosen. It
...
- Drop the per-disk reserve to zero instead. Rejected. It reaches a 55 GiB ceiling in one line, but
  the reserve is the only thing standing between replica scheduling and the operating system, the nix
  store and containerd's image store on the same disk.
...
- Alert on the `Scheduled` condition only and drop the headroom alerts as unreliable. Rejected. The
  condition is reactive and fires once a replica has failed to place, where the headroom alerts are
  predictive, and 0085 exists because that prediction was wanted and silently absent.
...
and by `NodeFilesystemAlmostOutOfSpace`, `PersistentVolumeFillingUp` and `PersistentVolumeAlmostFull`
for what is written: a provisioning ceiling that tracked nothing real exchanged for one that does.

Single-sourcing the over-provisioning literal and pinning it with a promtool case is a mitigation
rather than a fix, and the pattern worth carrying forward from this record and from 0085 is what
makes it tolerable: a guardrail derived from a value that can move should be paired with one that
reads the system's own answer, as `LonghornDiskUnschedulable` does here.
...
lands on. At 20Gi zot's fits everywhere, so the symptom is gone and the mechanism is not, and
`WaitForFirstConsumer` remains unverified against Longhorn's CSI driver.
