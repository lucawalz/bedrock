...
Rebuilding a node or recovering from a failure should mean reading the repository and applying it, not remembering what was done by hand. That requires one reviewable source of truth for both layers, the machines and the workloads on top of them. Two layers, two tools, the same principle.
...
Changes are reviewable and history is meaningful, because the repository is the only way state reaches the cluster. The cost is a learning curve and a stricter discipline: a broken commit on `main` is a broken cluster, so CI checks the manifests and the Nix evaluation before anything merges.
