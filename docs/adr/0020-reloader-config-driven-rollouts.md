...
A change to a watched ConfigMap or Secret now rolls the annotated workloads on its own. Editing the cloudflared routing table or a homepage config file is a single committed change that reaches the running Pods without a manual restart. The same path covers secret rotation: when a SOPS-managed Secret is re-encrypted and reconciled, the workloads that mount it and opt in will roll to pick it up, which removes a manual step from future credential rotation.
