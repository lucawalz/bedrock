...
The blog's Traefik router label is generated as `<namespace>-<ingressroute>-<hash>@kubernetescrd`, where the hash is not known until Traefik renders the route, so the rules match it by the `blog-blog-` prefix. The prefix is stable; the matcher must be confirmed against live metrics once Traefik is scraping, and tightened if a second route shares the prefix.
...
The router-label matcher is the one piece that cannot be fully verified offline. Until Traefik is scraping, the `blog-blog-` prefix is asserted from the IngressRoute naming convention, and the rules will silently produce no data if the rendered label differs; confirming it against live metrics after rollout is a required follow-up. The 30d windows also mean the budget-remaining figures and the slow-burn ticket alert only become meaningful after the series has thirty days of history, and read as full until then.
