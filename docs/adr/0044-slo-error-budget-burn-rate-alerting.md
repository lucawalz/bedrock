...
- Hand-written recording rules and multi-burn-rate alerts, chosen. The whole SLO is a single PrometheusRule that any reader can audit against the workbook. It is validated offline with `promtool check` and a `promtool test` unit fixture that asserts a burn-rate alert fires at its threshold and the budget rule computes. It adds no controller and no new failure surface, and it reuses the recording-rule and ServiceMonitor patterns already in the repo.
...
Traefik now exposes a metrics endpoint, and the blog has measurable, alertable objectives whose alerts fire on budget burn rate. A brief blip stays quiet while sustained degradation pages quickly and slow erosion opens a ticket. The pattern is the template for every later SLO: add a ServiceMonitor if the target is not already scraped, then a PrometheusRule of recording rules and burn-rate alerts, then a dashboard ConfigMap.
