...
The cluster runs on three Lenovo ThinkCentre m920q mini PCs with limited CPU and memory, so the control plane has to leave room for actual workloads. The hosts are already declared in NixOS, so whatever Kubernetes distribution is chosen has to install and upgrade cleanly from a NixOS module.
...
The overhead is low and node configuration stays declarative. The cost is that K3s bundles components this project does not want: servicelb, Traefik, local-storage, CoreDNS and metrics-server are all turned off with `--disable`, and their replacements are wired in through Flux.
