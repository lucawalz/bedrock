...
Standing up the peer surfaced where the second cloud genuinely leaks in, which is the lesson worth recording. The default `t3.small` node hit the ENI-based pod limit before the blog could schedule, so the pool is `t3.medium`. The controller could not auto-discover the VPC from instance metadata because the pod's hop to IMDS is blocked by the node's default hop limit; the textbook remedy of raising the hop limit through a launch template is not available here, since attaching a custom launch template to an EKS managed node pool makes CAPA demand bootstrap data the managed pool does not provide. The controller therefore runs with `hostNetwork` and `ClusterFirstWithHostNet` so it reaches IMDS through the node directly, and a hardcoded VPC id was deliberately rejected as instance-specific. With `hostNetwork` only one controller replica can bind the host port per node, so the deployment is pinned to a single replica. The IRSA role's trust must name the actual service account, which Flux prefixes with the release name (`kube-system-aws-load-balancer-controller`), not the bare chart name. These are the real cost of a second cloud, not the GitOps wiring, which carried over unchanged.
