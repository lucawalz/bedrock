...
**Update 2026-08-31:** the tunnel carries four hosts. `grocy.syslabs.dev` joined under [0079](0079-public-grocy-with-scoped-api-bypass.md), so that a phone can reach the grocery inventory from a shop. Its browser interface is behind Cloudflare Access with multi-factor authentication and Authentik forward auth; a second, narrow Access application bypasses only the `/api/` prefix, which the native client needs and which Grocy protects with an application-issued API key. That is the same shape as the rancher exposure and the same compensating control.
