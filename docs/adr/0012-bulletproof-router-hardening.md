...
**Update (2026-06-15):** WireGuard was retired the same day by [0023](0023-tailscale-overlay.md), which replaces it with Tailscale. "The trusted cluster and WireGuard zones" above names a zone that no longer exists; only the cluster zone remains trusted from that pair.
...
The inter-zone surface is small and observable, at the cost of more configuration to write and maintain. The zoned nftables firewall with default-deny between zones is in place. The cluster on VLAN 20 and the Tailscale overlay are trusted, while the VLAN 30 DMZ may reach only the internet and is dropped toward both the cluster and the home LAN. The home-LAN-to-DMZ drop is ordered ahead of the NAT catch-all, so the catch-all does not shadow it. The home LAN keeps split-horizon web access to the service VIP `10.20.0.50` on ports 80 and 443 only, and can no longer open new connections to the cluster nodes' SSH, kubelet, etcd, or API surface. Admin access to the cluster runs over the Tailscale overlay. Router management ports, SSH and the AdGuard interface, are bound to VLAN 20 and are not reachable from the WAN or the home LAN. The AdGuard admin login is protected by a bcrypt credential delivered through the agenix secrets model from [0007](0007-agenix-sops-secrets.md). No intrusion detection system or behavioral blocker is deployed: Suricata and CrowdSec are not in place, and any such layer remains future work. The honest limit is that volumetric denial-of-service cannot be solved at home, because it is an upstream problem that no edge configuration can absorb. The hardening builds on the NixOS router from [0003](0003-nixos-router-over-opnsense.md), and the zone definitions it enforces are recorded in [0016](0016-concrete-zoned-ip-scheme.md).

**Update (2026-07-30):** the sentence above stating "the cluster on VLAN 20 and the Tailscale overlay are trusted" is superseded by the Decision's update of the same date. The Tailscale overlay is no longer trusted toward VLAN 20; forwarding from the tailnet is an explicit port allowlist. Only the cluster zone remains trusted in the sense this paragraph describes.
