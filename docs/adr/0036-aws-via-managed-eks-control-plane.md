...
AWS credentials are a dedicated, scoped identity rather than the account root. clusterawsadm bootstraps the CAPA IAM roles and a `bootstrapper` user, and that user's access key, encrypted with SOPS, is the only credential the repository holds. The first cluster, `aws-1`, is authored as plain manifests and reconciles: a single managed node group on a small instance in `eu-central-1`, with the control-plane and node-group IAM roles bound by name to the ones the bootstrap created. The EKS control plane bills for as long as the cluster exists, so a cluster is brought up for use and torn down when idle rather than left running.
