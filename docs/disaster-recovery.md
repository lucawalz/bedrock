...
| A volume deleted or corrupted in place | Only for the volumes the snapshot job covers, and only back to the last nightly snapshot | The `snapshot` RecurringJob is scoped to the `default` recurring-job group, which covers 6 of the cluster's 19 volumes: the four paperless volumes, open-webui and home-assistant-config. Grafana, minio, pgadmin, ollama, kiwix-library, data-zot-0, loki, tempo, prometheus and the three postgres volumes carry no such label and have no snapshot at all. Where it runs, the job takes a snapshot at 03:30 and `snapshot-prune` retains the last 7; replication alone copies the write to all three replicas and cannot undo it |
...
| Longhorn volumes | Three-way replication on the retain class, plus a nightly `snapshot` job retaining 7 for the 6 of 19 volumes in the `default` recurring-job group (the four paperless volumes, open-webui, home-assistant-config) | Zero for a lost disk or node on any volume; for a deletion or a corruption, up to 24 hours and none beyond a week on a covered volume, no recovery point at all on the other 13 | Automatic replica rebuild for a disk or a node; a manual revert to the snapshot for a deletion or a corruption, where a snapshot exists |
...
- The `snapshot` recurring job covers only 6 of the 19 Longhorn volumes, because it is scoped to the `default` recurring-job group and most volumes were never put in it. Only the four paperless volumes, open-webui and home-assistant-config carry the group label. Grafana's SQLite, MinIO, pgAdmin, Ollama, kiwix-library, data-zot-0, loki, tempo, prometheus and the three postgres volumes have no snapshot at all. This is a live gap in the mechanism that exists now, not a historical one, and the coverage question should be settled deliberately rather than inherited.
